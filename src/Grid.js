    ...
    return (
        <ul className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-10 align-middle justify-items-center items-center">
            ...
            return (
              <li
                key={index}
                className="bg-slate-200 outline outline-offset-2 outline-4 outline-blue-600 rounded-xl text-center relative w-36 h-36 md:w-52 md:h-52 xl:w-64 xl:h-64"
              >
                <a
                  href={resource.link}
                ...
                <div
                  className="h-full w-full aspect-square block absolute top-0 left-0 rounded-xl transition-opacity duration-300 opacity-50 lg:opacity-0 hover:opacity-100 bg-slate-800/75 z-10"
                >
